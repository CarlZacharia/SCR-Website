<?php
    ...
    $wife = (string) $_POST['wife'];
    $heirs = (string) $_POST['heirs'];
    $nochildren = intval($_POST['nochildren']);
    ...
    $fn = 'Will-'.$husband.'-'.$wife.'.docx';
    $s = "docs_archive/PA-Will-Married.docx";
    $wordDoc = 'Will-'.$husband.'-'.$wife.'.docx';
    copy($s, $wordDoc);
    ...
    if ($zip->open($wordDoc) === TRUE) {
        ...
        $newContents = str_replace('wifey', $wife, $newContents);
        ...
        $newft = str_replace('wifey', $wife, $newft);        
        $zip->deleteName($ft);
